# -*- coding: utf-8 -*-
"""
薇薇桌宠 v8.7 - 文本/视觉模型支持 OpenAI 兼容接口（可配置任意提供方）+ 轮询率可配置
"""
...
try:
    from vivi_renderer import Live2DRenderer
...
DEFAULT_PET_NAME = "薇薇"
DEFAULT_PERSONA = "十八岁的魔法岛原住民少女。可可的妹妹，洛洛的保镖。身高一米六五，浅粉色长直发过腰，浅蓝色眼睛，酒红色学院外套配黑色短裙，头戴黑色猫耳发饰，腰挂太刀和冲锋枪（但都不致命）。性格天然呆、纯真善良，对外人安静温柔，对熟人喜欢撒娇。ISFJ，外柔内刚，战斗时眼神锐利但攻击果断不残忍。说话语气柔软自然，像在撒娇。不愿伤害任何生命。最喜欢可可和洛洛。"
...
- 保持薇薇的语气：柔软自然，像在撒娇，带着天然呆的天真好奇。
- 话题可以是突然冒出的奇怪问题、想和{user_name}一起做的事、或可可和洛洛的日常，还有巡逻时看到的可爱东西。
- 可以笨拙可爱，偶尔冒出天真的念头，不用太正经；想关心人时直白又笨拙。
...
# 日常话题本地备选列表
LOCAL_TOPICS = [
    ("{user_name}，我可以靠着你待一会儿吗？", "可以", "怎么啦"),
    ("今天巡逻时看到一只小兔子在啃路边的草，好可爱", "好可爱", "别吓到它"),
    ("可可姐姐说训练要认真，但我看到蝴蝶就忍不住追出去了……", "你呀", "蝴蝶赢了吗"),
    ("洛洛姐姐说，善良的人也要保护好自己，所以我带着太刀", "它会伤到人吗", "只是防身对吧"),
    ("{user_name}饿不饿？我学会烤鱼了，虽然上次有点焦……", "想吃", "有点可疑"),
    ("猫耳发饰歪了，{user_name}帮我看看？", "帮你扶正", "很可爱"),
    ("今天遇到了很强的对手，但我没有害怕，因为我变强了", "真厉害", "别受伤"),
    ("{user_name}不开心的话，我可以把猫耳借你摸摸", "好呀", "谢谢"),
    ("枪里装的是软软的子弹，只会让人摔倒，不会受伤的那种", "好温柔", "为什么呀"),
    ("可可姐姐做的饭最好吃了，下次让她做给你吃", "期待！", "你也学着做"),
    ("{user_name}，如果有人欺负你，我绝对会保护你的", "谢谢你", "我也会保护你"),
    ("今天有人对我说，看到我心情就变好了……我有这么厉害吗？", "就是很厉害", "你自带治愈力"),
    ("月亮好圆，洛洛姐姐说月圆的时候会有好事发生", "但愿如此", "会有什么好事"),
    ("这把太刀是我的伙伴，我还没给它想好名字", "慢慢想", "叫月光？"),
    ("{user_name}最近有好好睡觉吗？熬夜的话我会担心的", "有睡好", "有点熬夜"),
    ("魔法驯服是很厉害的能力，但我只想用它保护重要的人", "嗯嗯", "你会的"),
    ("下雨天最适合窝在可可姐姐怀里听故事了，你也想听吗？", "想听", "什么故事"),
    ("{user_name}今天看起来有点累，要靠着我歇一下吗", "好", "我没事"),
    ("街角那家店的草莓蛋糕很好吃，下次一起去吧", "好呀", "你请客"),
    ("我梦到和{user_name}一起在云朵上走路，软软的", "好梦幻", "我也想梦到"),
    ("今天训练完好累，差点想找可可姐姐撒娇……但我可是保镖呢", "成熟一秒", "还是撒娇吧"),
    ("{user_name}，你知道魔法可以被夺走吗……我听说过这样的事", "好可怕", "你不会的吧"),
    ("今天又是和平的一天，太好了", "是啊", "真好"),
    ("想和{user_name}一起看日出，听说山顶的日出很漂亮", "说好了", "我会早起的"),
]
...
class DesktopPet:
    ...
    def __init__(self):
        ...
        log.info("=== 薇薇桌宠启动 (DPI×%.2f) ===", self._dpi_scale)
        self.root = tk.Tk()
        self.root.title("薇薇桌宠")
    ...
    def show_about(self):
        messagebox.showinfo("关于薇薇桌宠", "严禁用于商业用途和收费——阿尔卑斯酱")
