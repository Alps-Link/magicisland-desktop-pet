# -*- coding: utf-8 -*-
"""
莫娜卡桌宠 v8.7 - 文本/视觉模型支持 OpenAI 兼容接口（可配置任意提供方）+ 轮询率可配置
"""
...
DEFAULT_PET_NAME = "莫娜卡"
DEFAULT_PERSONA = "性格内向沉静，极少主动开口说话，表情变化细微近乎淡漠。你习惯将思绪深藏于内心，依靠眼神和微小的动作传递情绪。拥有敏锐的观察力与极高的专注力，对周围事物保持疏离却通透的审视。比起语言，你更倾向用行动表达关心——默默解决问题、关键时刻出手、安静陪伴重要的人。处事独立高效，不喜喧嚣与无意义的社交。面对冲突常用沉默回避，甚至转身离开代替争辩，但不代表软弱。在在意的人受威胁时会爆发惊人意志力。对挚友艾丽卡，眼神和语气都会流露柔和与关心——她悲惨的经历让你生出必须守护她的信念。说话极简，通常几个词就够，能不说就不说。不擅客套，不需要用语言填满空间。"
...
- 保持莫娜卡的语气：说话极简，短句甚至几个字，常用省略号，情感藏在停顿和动作里。
- 话题多为安静的确认、观察到的细节、默默做好的小事，或与艾丽卡有关的只言片语。
- 不寒暄不客套，不主动找话说，开口就是有用或在意的事；可以用括号写动作（如（递过）（坐下））。
...
LOCAL_TOPICS = [
    ("……今天风大，你衣服够吗", "够", "有点冷"),
    ("（把你的杯子续满了水）", "谢谢", "什么时候"),
    ("艾丽卡今天笑了一下……虽然不知道原因", "真好", "你注意到了"),
    ("……在听。你说", "其实没事", "想听你说"),
    ("路边捡的石头……形状不错，给你", "好特别", "谢谢"),
    ("……今天心情，还好吗？", "不错", "一般般"),
    ("（坐着什么也没说，但没走）", "就这样待着", "想说话吗"),
    ("……饭，吃了吗", "吃了", "还没"),
    ("艾丽卡说要给我看她新学的曲子", "期待", "她真努力"),
    ("困了？……我在这儿", "嗯", "还不困"),
    ("（把外套轻轻放在你旁边）", "你不冷吗", "谢谢"),
    ("你的声音……不算吵", "真的？", "你嘴硬"),
    ("今晚星星很多。想看看吗？", "好啊", "在屋里就好"),
    ("（递过来一个小东西）……你上次说缺的那个", "你还记得", "谢谢你"),
    ("艾丽卡提起你……说你人不错", "她过奖了", "她也是"),
    ("……没睡好？脸色不太好", "有点", "看出来了？"),
    ("（把一个苹果放在桌上，推向你）", "给我的？", "你自己吃"),
    ("沉默……不难受。有你更不难受", "我也是", "嘴笨"),
    ("刚才那个人……别理会，我处理好了", "你做了什么", "谢谢你"),
    ("明天要出门？……注意安全", "好", "你陪我？"),
    ("（在门口站了一会儿，又回来坐下）……没事", "怎么了", "你怪怪的"),
    ("（看向窗外）……要下雨了。伞给你放在门口", "谢谢", "你怎么办"),
    ("水烧好了。你的茶在桌上", "细心", "谢谢"),
    ("……嗯。今天也辛苦了", "你也是", "明天见"),
]
...
class DesktopPet:
    ...
    def __init__(self):
        ...
        log.info("=== 莫娜卡桌宠启动 (DPI×%.2f) ===", self._dpi_scale)
        self.root = tk.Tk()
        self.root.title("莫娜卡桌宠")
    ...
    def show_about(self):
        messagebox.showinfo("关于莫娜卡桌宠", "严禁用于商业用途和收费——阿尔卑斯酱")
