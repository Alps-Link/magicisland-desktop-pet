...
def _render_offscreen(bridge, width, height):
    ...
    model.LoadModelJson(os.path.join(MODEL_DIR, 'Invi.model3.json'))
    ...
    scene_params = {}
    m3_path = os.path.join(MODEL_DIR, 'Invi.model3.json')
    ...
    first_impression_end = time.time() + 3.983  # FirstImpression 3.983s：自然播完（Loop=false）后切待机
            ...
            while cmd_idx < len(bridge._cmd_q):
                ...
                if cmd[0] == 'quit': bridge._ready = False; break
                    ...
                    scene_map = {
                        'happy': 24,      # exp_06
                        'shy': 25,        # exp_07
                        'surprised': 15,  # 16.motion3
                        'worried': 2,     # 03.motion3
                        'hurried': 11,    # 12.motion3
                        'sleep': 3,       # 04.motion3
                    }
