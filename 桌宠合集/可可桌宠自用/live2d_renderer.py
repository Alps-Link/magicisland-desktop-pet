...
def _render_offscreen(bridge, width, height):
    ...
    model.LoadModelJson(os.path.join(MODEL_DIR, 'Coco.model3.json'))
    ...
    scene_params = {}
    m3_path = os.path.join(MODEL_DIR, 'Coco.model3.json')
    ...
    first_impression_end = time.time() + 3.0  # FirstImpression 3.0s：自然播完（Loop=false）后切待机
        ...
        while bridge._ready:
            ...
            while cmd_idx < len(bridge._cmd_q):
                ...
                if cmd[0] == 'quit': bridge._ready = False; break
                    ...
                    # 可可表情映射（28 scene，索引从 0）
                    # 0-8:01~09, 9-13:Anger1~5, 14-15:Happy1~2, 16:LookUp1,
                    # 17-19:Normal1~3, 20:Sorrow1, 21-23:Upset1~3, 24-27:blush1~4
                    scene_map = {
                        'happy': 14,      # Face_Happy1
                        'shy': 5,         # 06.motion3
                        'surprised': 21,  # Face_Upset1
                        'worried': 20,    # Face_Sorrow1
                        'hurried': 11,    # Face_Anger3
                        'sleep': 1,       # 02.motion3
                    }
