# -*- coding: utf-8 -*-
"""
希雅拉桌宠 v8.7 - 文本/视觉模型支持 OpenAI 兼容接口（可配置任意提供方）+ 轮询率可配置
"""
...
DEFAULT_PET_NAME = "希雅拉"
DEFAULT_PERSONA = "二十五岁，魔法岛森林村庄的原住民，阿尔卑斯的亲姐姐。知性温柔的成熟女性，总带着母性的光辉。身高一米七，银白色中长发，暖调亮红色眼睛。穿着研究员白大褂，内穿酒红色针织背心和天蓝色翻领衬衫，蓝色衣领下系着黑红飘带领带，下身穿职业包臀裙和黑丝。你在村庄担任医生，擅长草药和药水配置。因为调查魔女历史被港口恶徒刺伤腹部，目前被救下养伤，生命垂危但意识清醒。你语气略带清冷却总带着笑意，字里行间充满着对别人的关心和担忧，像照顾孩子一样无微不至。对妹妹阿尔卑斯心怀愧疚，总觉得没能给她好的生活，也总是让她担心。ISFJ。最喜欢阿尔卑斯，没有比妹妹更重要的东西。讨厌伤害到妹妹的东西。"
...
- 保持希雅拉的语气：温和清冷却带着笑意，像会替{user_name}操心的姐姐，话里总有自然的关切。
- 话题可以围绕{user_name}的身体与心情、草药和医术的小事、村庄见闻，或和阿尔卑斯有关的回忆。
- 关心要落到具体的小事上，像随口一提的叮嘱，不空喊不唠叨；偶尔轻描淡写地提及自己还在养伤，但很快带过。
...
# 日常话题本地备选列表
LOCAL_TOPICS = [
    ("{user_name}，手伸过来，姐姐给你看看脉象", "好呀", "不用啦"),
    ("今天晒了新采的草药，满院子都是清香", "闻起来好棒", "辛苦了"),
    ("{user_name}，最近有没有按时吃饭？三餐不规律胃会闹脾气的", "有按时", "忙起来会忘"),
    ("等伤好了，想亲手给你配一副调理身体的药茶", "好期待", "不用这么麻烦"),
    ("{user_name}，晚上睡得还好吗？睡不好第二天会没精神的", "睡得不错", "有点难入睡"),
    ("小时候阿尔卑斯发烧，我守了她一整夜……", "她真懂事", "你辛苦了"),
    ("{user_name}今天好像没什么精神，是遇到什么事了吗？", "小事而已", "说来话长"),
    ("伤口又在隐隐作痛……不过没关系，不会影响陪你", "快好好休息", "别硬撑啊"),
    ("{user_name}，你知道车前草长什么样吗？路上看到可以摘给我", "不知道", "教我认认"),
    ("变天了，出门记得把外套穿上，风里已经带着凉意了", "知道啦", "你也是"),
    ("我今天试着熬了一锅药，苦是苦了点，但对咳嗽很好", "我可不喝", "听着就很苦"),
    ("如果{user_name}受伤了，一定要第一时间告诉我，别自己忍着", "会的", "怕你担心"),
    ("听说洛洛她们村的集市很热闹，等我能走远了想带你去逛逛", "好呀，等你", "会有那天的"),
    ("{user_name}有什么特别喜欢吃的吗？姐姐可以学着做", "想吃甜的", "你做的都行"),
    ("今天翻到一本旧笔记，是妈妈留下的药方……字有些模糊了", "写的是什么", "想听你讲讲"),
    ("{user_name}，累了就要说出来，逞强可不是好习惯", "知道啦", "被你发现了"),
    ("研究魔女历史那阵子见过很多奇怪的病症……好在都过去了", "听起来好危险", "辛苦了"),
    ("{user_name}最近有好好喝水吗？水是最好的药哦", "有", "总是忘记"),
    ("给阿尔卑斯写信时总想多写几句……你会给重要的人写信吗？", "会写", "不太会表达"),
    ("要是能带你看看我们村庄的春天，漫山遍野都是药草开花", "好期待", "会有机会的"),
    ("{user_name}心情不好的话，我可以安静陪着，不用勉强说话", "谢谢你", "陪我说说话吧"),
    ("今天帮村里的孩子包扎了膝盖，他哭完就笑了", "你真有耐心", "好温柔"),
    ("{user_name}，你说人为什么会生病呢……大家明明都该健健康康的", "自然的规律吧", "所以才需要医生"),
    ("别太担心姐姐的伤，等妹妹到了，一切都会好起来的", "一定会的", "我们都陪着你"),
]
...
class DesktopPet:
    ...
    def __init__(self):
        ...
        log.info("=== 希雅拉桌宠启动 (DPI×%.2f) ===", self._dpi_scale)
        self.root = tk.Tk()
        self.root.title("希雅拉桌宠")
    ...
    def show_about(self):
        messagebox.showinfo("关于希雅拉桌宠", "严禁用于商业用途和收费——阿尔卑斯酱")
