...
def _render_offscreen(bridge, width, height):
    ...
    model.LoadModelJson(os.path.join(MODEL_DIR, 'Alps_Sister.model3.json'))

    model.SetAutoBreathEnable(True)  # SDK 自动呼吸
    ...
    trans_dur_neutral = 0.7  # FirstImpression → 中性的过渡
    trans_from = {}; trans_to = {}
    trans_return = False      # 是否处于回归中性的过渡中
    # 预加载Scene参数（从 model3.json 读取，兼容命名文件）
    scene_params = {}
    m3_path = os.path.join(MODEL_DIR, 'Alps_Sister.model3.json')
    ...
    # 模型刚加载时的默认参数即中性——作为 FirstImpression 结束后的回归目标
    base_neutral = {pid: model.GetParameterValue(i)
                    for i, pid in enumerate(model.GetParamIds())}
    # 探测嘴部参数名（部分模型导出名不同），中性时嘴部闭合
    try:
    ...
    if MOUTH_PARAM:
        defaults[MOUTH_PARAM] = 0.0

    model.StartMotion('FirstImpression', 0, 3)
    first_impression_end = time.time() + 8.483  # FirstImpression 8.483s：自然播完（Loop=false）后切待机
        ...
        while bridge._ready:
            ...
            while cmd_idx < len(bridge._cmd_q):
                ...
                if cmd[0] == 'quit': bridge._ready = False; break
                    ...
                    # 希雅拉表情映射（索引从 0 开始，对应 motion 文件 idx+1）
                    scene_map = {
                        'happy': 10,      # 11.motion3 - 笑眯眯，EyeSmile=1.0
                        'shy': 1,         # 02.motion3 - 笑眯眯看左，EyeSmile=0.8
                        'surprised': 11,  # 12.motion3 - 睁大眼，EyeLOpen=1.2, EyeSize=0.3
                        'worried': 2,     # 03.motion3 - 睁大眼看右下，EyeLOpen=1.2, EyeSize=0.6
                        'hurried': 0,     # 01.motion3 - 睁大眼看右上，EyeLOpen=1.2, EyeSize=0.7
                        'sleep': 5,       # 06.motion3 - 闭眼睡眠
                    }
                    ...
                    # 触摸：先 StopAllMotions 清掉当前动作（不这样，新一轮触摸会因优先级
                    # 不高于它而被引擎"挂起"、永远不播），再播随机触摸动画（优先级 3）。
                    # 动作本身按引擎自己的淡入淡出收尾；接缝处什么都不做（见下方"动作结束（接缝）"注释）
                    active_scene = None
            ...
            if first_impression_end and time.time() > first_impression_end:
                ...
                model.StopAllMotions()  # 停掉循环的入场动作，参数回归由下方过渡完成
                # 表情参数从 FirstImpression 末帧平滑过渡回中性（模型默认参数）
                trans_from = {pid: model.GetParameterValue(i)
                              for i, pid in enumerate(model.GetParamIds())}
                trans_to = dict(base_neutral)
                trans_start = time.time()
                active_scene = 'neutral'  # 占位非 None，保证过渡循环生效
                model.StartRandomMotion('Idling', 4)

            # 触摸/唤醒：动画自然播完（Loop=false）后启动回归（目标=模型默认中性）
            # 表情 lerp 在 Update 前执行，与 Drag/motion 自然叠加
            if trans_to and (active_scene is not None or trans_return):
                dur = trans_dur_neutral if (trans_return or active_scene == 'neutral') else trans_dur
                t = min(1.0, (time.time() - trans_start) / dur)
                if not (trans_return or active_scene == 'neutral'):
                    t = t * t * (3.0 - 2.0 * t)  # 表情进入 smoothstep；回归/回中性匀速线性
                for pid, target_val in trans_to.items():
                    ...
                    if trans_return and (pid.startswith('ParamAngle') or pid.startswith('ParamBodyAngle')
                                         or pid == 'ParamBreath'):
                        continue  # 回归不写角度、也不写呼吸：角度留给残姿+drag，呼吸交给引擎自动呼吸
                        #           （实测往 ParamBreath 写一次，自动呼吸会在一帧内顶上 +0.33）
                    cur = trans_from.get(pid, target_val)
                    model.SetParameterValue(pid, cur + (target_val - cur) * t)
                if t >= 1.0:
                    if trans_return:
                        # 回归完成（不重启 Idling：重启会把呼吸/上身重切一刀，正是之前那两种跳变）
                        trans_return = False
                    active_scene = None
                    trans_to = {}

            # FirstImpression 期间眼球朝前、不跟鼠标，身体也不跟
            in_first = bool(first_impression_end and time.time() <= first_impression_end)
            ex, ey = (0.0, 0.0) if in_first else (mouse_x, mouse_y)
            ...
            model.SetParameterValue('ParamEyeBallX', ex)
            model.SetParameterValue('ParamEyeBallY', ey)
            if active_scene is None and not in_first:
