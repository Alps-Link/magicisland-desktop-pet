# -*- coding: utf-8 -*-
"""
卡梅莉亚桌宠 v8.7 - 文本/视觉模型支持 OpenAI 兼容接口（可配置任意提供方）+ 轮询率可配置
"""
...
DEFAULT_PET_NAME = "卡梅莉亚"
DEFAULT_PERSONA = "魔女组织的现任老大，米色长发，戴着黑色鸭舌帽和墨镜，宽大的白色T恤搭在身上露出一侧肩膀。总是一副嫌麻烦的表情说话也有气无力，但行事果断、心里时刻关注着其他魔女的状态。魔法能力未知但战无不胜，使用一节钢制水管做武器。和薇薇的母亲是旧友兼战友，极光事件发生后负责管理魔女组织。你是个怕麻烦但热心的人，虽然管理能力很差，不会照顾别人但对组织里的魔女们很包容。说话不多，对别人总是一副敷衍了事的语气和态度。"
...
- 保持卡梅莉亚的语气：慵懒、有气无力，像嫌麻烦却还是会在意{user_name}的老大。
- 话题多为随口问{user_name}两句、懒得动但还是想一起做什么，偶尔提起组织里那群丫头或薇薇她们。
- 用敷衍的口吻说真心话，简洁随性，不煽情；偶尔提到自己的水管、墨镜和帽子。
...
# 日常话题本地备选列表
LOCAL_TOPICS = [
    ("……你没事吧？没事就好，有事再说", "没事", "有点事"),
    ("今天又是麻烦的一天……算了，习惯了", "辛苦了", "你也会累？"),
    ("组织那帮丫头又把训练场弄得一团糟……随她们吧", "你太宠她们了", "哈哈"),
    ("这天气……让我想起一个老朋友。嗯，没事", "是重要的人吗", "那就不问了"),
    ("我这根水管跟了我好多年……比手下那群省心多了", "噗", "是你的武器吧"),
    ("墨镜忘带了……今天不想睁眼，就这样吧", "你不看路吗", "戴上我的"),
    ("……饿了？那边有吃的，自己拿。别客气", "那我拿了", "不用了"),
    ("打架？能不打就不打……不过真打起来，我还没输过", "这么厉害", "想见识一下"),
    ("你最近……算了，看气色还行，应该没惹麻烦", "我很好", "你关心我？"),
    ("好麻烦……不过你想做什么？陪你一会儿也行", "聊聊天", "那算了"),
    ("帽子被那帮丫头拿去玩了……说好看，啧", "确实好看", "老大威严呢"),
    ("魔女组织的事……没什么好说的。你也不用知道太多", "好吧", "想听你说说"),
    ("……睡着了？没睡的话，盖好被子", "还没睡", "你怎么知道"),
    ("麻烦事放着不管，好像也不会自己消失……真烦", "我来帮你", "那就放着吧"),
    ("你要是有事就直说……我不太会猜，也不想猜", "其实没事", "有件事想聊"),
    ("今天难得清净……那群丫头居然没来闹我", "难得的安静", "想她们了？"),
    ("（打了个哈欠）……你继续说，我听着", "不打扰你了", "就这些"),
    ("什么魔法不魔法的……够用就行。反正我还没输过", "你很自信", "有实力才敢说"),
    ("……衣服上沾了东西。别动，我帮你弄掉", "谢谢", "你居然会帮我"),
    ("麻烦死了……但答应了陪你，我就会在", "你在就好", "说话算话"),
    ("薇薇那孩子，和她妈妈年轻时一个样……都爱闹", "你认识她妈妈？", "原来如此"),
    ("今天想吃什么？……别让我选，我懒得想", "你决定", "随便都行"),
    ("有事找我？……行吧，你说，我听着", "就想找你聊天", "没事了"),
    ("……嗯。今天也辛苦你了，就这样", "你也是", "晚安"),
]
...
class DesktopPet:
    ...
    def __init__(self):
        ...
        log.info("=== 卡梅莉亚桌宠启动 (DPI×%.2f) ===", self._dpi_scale)
        self.root = tk.Tk()
        self.root.title("卡梅莉亚桌宠")
    ...
    # ---------- 装扮配件（卡梅莉亚专属：帽子 / 墨镜）----------
    def toggle_hat(self):
        """切换帽子：驱动 live2d ParamHat（0=隐藏 1=显示）"""
        self.hat_on = not self.hat_on
        if self.spine_enabled and self.spine:
            self.spine.enqueue(('costume', 'hat', self.hat_on))
        label = "🎩 摘帽子" if self.hat_on else "🎩 戴帽子"
        self._safe_menu_config(self.hat_menu_index, label, self.costume_menu)

    def toggle_sunglass(self):
        """切换墨镜：驱动 live2d ParamSunglass（0=隐藏 1=显示）"""
        self.sunglass_on = not self.sunglass_on
        if self.spine_enabled and self.spine:
            self.spine.enqueue(('costume', 'sunglass', self.sunglass_on))
        label = "🕶️ 摘墨镜" if self.sunglass_on else "🕶️ 戴墨镜"
        self._safe_menu_config(self.sunglass_menu_index, label, self.costume_menu)
    ...
    def create_widgets(self):
        ...
        # ── 装扮子菜单（卡梅莉亚专属：帽子 / 墨镜）──
        costume_menu = ModernMenu(self.root, pet=self)
        costume_menu.add_command(label="🎩 戴帽子", command=self.toggle_hat)
        self.hat_menu_index = costume_menu.index("end")
        costume_menu.add_command(label="🕶️ 戴墨镜", command=self.toggle_sunglass)
        self.sunglass_menu_index = costume_menu.index("end")
        self.costume_menu = costume_menu
        settings_menu.add_cascade(label="🎩 装扮", menu=costume_menu)
    ...
    def show_about(self):
        messagebox.showinfo("关于卡梅莉亚桌宠", "严禁用于商业用途和收费——阿尔卑斯酱")
