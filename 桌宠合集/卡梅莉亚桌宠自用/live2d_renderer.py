"""
Live2D 离屏渲染（卡梅莉亚版，阿尔卑斯系架构）

与阿尔卑斯渲染器差异：
- 模型为 Carmelia.model3.json
- 卡梅莉亚模型没有 Scene 动作组，场景参数从 Expression 组读取（Face_01~11，多为单帧姿态）
- 没有 Sleep 动作组，睡眠用闭眼表情场景（Face_06，ParamEyeLOpen=0）代替
- 表情映射按 Face 文件的眼睛/眉毛参数选取（用户确认 Face 间差异很小，随意匹配）
"""
...
def _render_offscreen(bridge, width, height):
    ...
    model.LoadModelJson(os.path.join(MODEL_DIR, 'Carmelia.model3.json'))
    ...
    # 预加载场景参数：优先 Scene 组，没有则用 Expression 组（卡梅莉亚即此情况）
    scene_params = {}
    m3_path = os.path.join(MODEL_DIR, 'Carmelia.model3.json')
    with open(m3_path, 'r', encoding='utf-8') as _f:
        _md = json.load(_f)
    motions_ref = _md.get('FileReferences', {}).get('Motions', {})
    scene_files = motions_ref.get('Scene') or motions_ref.get('Expression') or []
    ...
    touch_durations = []
    try:
        for _it in (motions_ref.get('Touch') or []):
        ...
        pass
    hat_on = False          # 装扮配件状态（部分动作会驱动这些参数归零，需每帧强制）
    sunglass_on = False
    ...
    # 入场动作：Touch_Body（Touch 组索引 1，2.983s，Loop=false 自然播完）
    model.StartMotion('Touch', 1, 3)
    first_impression_end = time.time() + 2.983
        ...
        while bridge._ready:
            ...
            while cmd_idx < len(bridge._cmd_q):
                ...
                if cmd[0] == 'quit': bridge._ready = False; break
                    ...
                    # 卡梅莉亚表情映射（Expression 组 Face_01~11，索引 0-10）
                    # Face 间差异很小，按眼睛/眉毛参数选取：
                    #   Face_05 眼睛0.7+眉毛 → happy；Face_02 → shy；
                    #   Face_07 眼睛1.0 睁大 → surprised；Face_10 眼睑下垂 → worried；
                    #   Face_09 眯眼0.2 → hurried；Face_06 完全闭眼 → sleep
                    scene_map = {
                        'happy': 4,      # Face_05
                        'shy': 1,        # Face_02
                        'surprised': 6,  # Face_07
                        'worried': 9,    # Face_10
                        'hurried': 8,    # Face_09
                        'sleep': 5,      # Face_06（无 Sleep 动作组，用闭眼场景代替）
                    }
                    ...
                    # 装扮配件：ParamHat / ParamSunglass（0=隐藏 1=显示）
                    # 记录状态并每帧强制（Touch_Body/Breast 等动作会驱动这些参数归零）
                    if cmd[1] == 'hat':
                        hat_on = bool(cmd[2])
                    elif cmd[1] == 'sunglass':
                        sunglass_on = bool(cmd[2])
                elif cmd[0] == 'mouse':
            ...
            model.Update()
            # 装扮配件每帧强制：Touch_Body/Breast 等动作曲线会把参数拉向 0。
            # 必须在 Update 之后设置（Update 内的 motion 加算会稀释 Update 前的直写）
            # 开关都强制：开=1.0 显示，关=0.0 隐藏（摘除即时生效，无需等动画驱动）
            if hat_on:
                model.SetParameterValue('ParamHat', 1.0)
            else:
                model.SetParameterValue('ParamHat', 0.0)
            if sunglass_on:
                model.SetParameterValue('ParamSunglass', 1.0)
            else:
                model.SetParameterValue('ParamSunglass', 0.0)
