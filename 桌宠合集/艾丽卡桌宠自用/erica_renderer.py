...
def _render(bridge, width, height):
    ...
    model.LoadModelJson(os.path.join(MODEL_DIR, 'Bandage.model3.json'))
    ...
    try:
        with open(os.path.join(MODEL_DIR, 'Bandage.model3.json'), 'r', encoding='utf-8') as _f:
            _md = json.load(_f)
    ...
    defaults = [model.GetParameterValue(i) for i in range(len(_param_ids))]
    # 中性回归时嘴部闭合（部分模型默认嘴部参数为张开）
    if MOUTH_PARAM and MOUTH_PARAM in _param_ids:
        defaults[_param_ids.index(MOUTH_PARAM)] = 0.0
    # 眼球参数由鼠标追踪每帧接管，不参与回归插值（参数分层，避免对抗）
    # 角度参数参与回归，但回归期间冻结 Drag（不传鼠标偏移），避免与回归插值拉锯
    restore_ids = [pid for pid in _param_ids if pid not in ('ParamEyeBallX', 'ParamEyeBallY')]
    restore_defaults = [defaults[_param_ids.index(pid)] for pid in restore_ids]
    ...
    EMOTION_MAP = {
        'relaxed':   ('Expression', 2),
        'surprised': ('Expression', 4),
        'hurried':   ('Expression', 5),
        'happy':     ('Expression', 0),
        'shy':       ('Expression', 6),
        'worried':   ('Expression', 5),
        'sleep':     ('Expression', 3),
    }
    model.StartMotion('FirstImpression', 0, 3)
    intro_end = time.time() + 3.833
            ...
            while cmd_idx < len(bridge._cmd_q):
                ...
                if c[0] == 'quit': bridge._ready = False; break
                    ...
                    if emo == 'normal':
                        ...
                        restore_from = [model.GetParameterValue(_param_ids.index(pid)) for pid in restore_ids]
